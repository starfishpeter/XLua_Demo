...
namespace GreatClock.Common.ExcelToSO
{
    ...
    {
        //默认的配置路径 个人开发无需担心配置问题 但多人协同 如果表格不同可能要做区分处理
        public const string SETTINGS_PATH = "ProjectSettings/ExcelToScriptableObjectSettings.asset";

        private static readonly Regex reg_color32 = new(@"^[A-Fa-f0-9]{8}$");
        private static readonly Regex reg_color24 = new(@"^[A-Fa-f0-9]{6}$");

        //GUI创建窗口
        [MenuItem("Tools/Excel To ScriptableObject")]
        static void Excel2ScriptableObject()
        {
            ExcelToScriptableObject window = GetWindow<ExcelToScriptableObject>("Excel To ScriptableObject");
        ...
        static bool Process(ExcelToScriptableObjectSetting excel, bool generateCode)
        {
            ...
            {
                string msg = $"表格的文件名{excel.excel_name}不符合规则, 必须是首字母大写的类名";
            ...
            catch
            {
                ...
                string msg = $"打开表格{excel.excel_name}失败，因为进程被占用了，也许你应该先关掉你的Excel再试试";
                EditorUtility.DisplayDialog("Excel To ScriptableObject", msg, "OK");
                return false;
            }
            ...
            if (data == null)
            {
                string msg = $"读取表格{excel.excel_name}失败，不是可以读取的xlsx文件，你应该注意WPS和Excel格式的区别";
                EditorUtility.DisplayDialog("Excel To ScriptableObject", msg, "OK");
                return false;
            }
            ...
            foreach (DataTable table in data.Tables)
            {
                ...
                if (table.Rows.Count < Mathf.Max(global_configs.field_row, global_configs.type_row) + 1)
                {
                    ...
                    string msg = $"转换表格{excel.excel_name}失败，至少需要两行来确定他们的变量名和变量类型";
                    EditorUtility.DisplayDialog("Excel To ScriptableObject", msg, "OK");
                    return false;
                }
                ...
                {
                    EditorUtility.ClearProgressBar();
                    string msg = $"错误的Sheet名称{sheet.itemClassName}，他也需要是个类名，所以首字母要大写";
                    EditorUtility.DisplayDialog("Excel To ScriptableObject", msg, "OK");
                    return false;
                }
                object[] items;
                ...
                for (int i = 0, imax = items.Length; i < imax; i++)
                {
                    ...
                    {
                        EditorUtility.ClearProgressBar();
                        string msg = $"转换表格{excel.excel_name}失败，因为{fieldName}不是一个正确的变量名，必须以_或者字母开头";
                        EditorUtility.DisplayDialog("Excel To ScriptableObject", msg, "OK");
                        return false;
                    }
                ...
                {
                    EditorUtility.ClearProgressBar();
                    string msg = $"转换表格{excel.excel_name}失败，因为没有正确的变量名";
                    EditorUtility.DisplayDialog("Excel To ScriptableObject", msg, "OK");
                    return false;
                }
            ...
            if (generateCode)
            {
                ...
                content.AppendLine("//----------------------------------------------");
                content.AppendLine("//    这是一个自动生成的脚本                      ");
                content.AppendLine("//    如非必要请不要直接修改                      ");
                content.AppendLine("//----------------------------------------------");
                content.AppendLine();
                ...
                {
                    content.AppendLine(string.Format("{0}\t{1}", indent, serializeAttribute));
                    content.AppendLine(string.Format("{0}\tpublic {1}[] _{1}Items;", indent, sheet.itemClassName));
                    if (excel.use_public_items_getter)
                    {
        ...
        //正则表达式 字符串必须以一个大写字母开头，后面可以跟0个或多个大小写字母、数字或下划线字符
        static bool CheckClassName(string str)
        {
            return Regex.IsMatch(str, @"^[A-Z][A-Za-z0-9_]*$");
        }

        //必须以一个字母或下划线字符开头，后面可以跟0个或多个大小写字母、数字或下划线字符
        ...
        void OnGUI()
        {
            ...
            EditorGUILayout.LabelField(" 一些提示：");
            EditorGUILayout.LabelField("- Excel文件中，名称以#开头的sheet将不被读取");
            EditorGUILayout.LabelField("- Excel文件中，名称以.开头的sheet将不暴露其Get方法，其中仅被其他sheet使用的数据才会被读取");
            ...
            EditorGUILayout.BeginHorizontal(GUILayout.MinHeight(10f));
                ...
                EditorGUILayout.BeginVertical();
                    ...
                    GUILayout.Space(12f);
                        ...
                        global_configs.field_row = EditorGUILayout.IntField("变量行（从零开始）", global_configs.field_row);
                        global_configs.type_row = EditorGUILayout.IntField("类型行（从零开始）", global_configs.type_row);
                        global_configs.data_from_row = EditorGUILayout.IntField("数据行（从零开始）", global_configs.data_from_row);
                ...
                bool processAll = GUILayout.Button("转换全部", GUILayout.Width(100f), GUILayout.Height(30f));

                //GUILayout.FlexibleSpace();
                if (GUILayout.Button("添加新的Excel", GUILayout.Width(100f), GUILayout.Height(20f)))
                {
            ...
            for (int i = 0, imax = excel_settings.Count; i < imax; i++)
            {
                ...
                cachedGUIBGColor = GUI.backgroundColor;
                EditorGUILayout.LabelField("表格文件", string.IsNullOrEmpty(setting.excel_name) ? "(未选取)" : setting.excel_name);
                bool invalidExceFile = string.IsNullOrEmpty(setting.excel_name);
                if (invalidExceFile)
                {
                    GUI.backgroundColor = Color.green;
                }
                if (GUILayout.Button("选取", GUILayout.Width(48f)))
                {
                    string p = EditorUtility.OpenFilePanel("选取Excel文件，格式xlsx", ".", "xlsx");
                ...
                if (setting.dir_manual_edit)
                {
                    ...
                    string scriptDirectory = EditorGUILayout.TextField("解析脚本存放目录", setting.script_directory);
                    ...
                    string assetDirectory = EditorGUILayout.TextField("转换资产存放目录", setting.asset_directory);
                ...
                {
                    int scriptIndex = EditorGUILayout.Popup("解析脚本存放目录", setting.script_dir_index, mFolders);
                    int assetIndex = EditorGUILayout.Popup("转换资产存放目录", setting.asset_dir_index, mFolders);
                ...
                setting.name_space = EditorGUILayout.TextField("命名空间", setting.name_space);
                EditorGUILayout.EndVertical();
                GUILayout.Space(8f);
                bool isDirEditManually = EditorGUILayout.ToggleLeft("手动编辑", setting.dir_manual_edit, GUILayout.Width(92f));
                ...
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.BeginVertical();
                setting.use_hash_string = EditorGUILayout.ToggleLeft("字符串统一存储", setting.use_hash_string);
                setting.hide_asset_properties = EditorGUILayout.ToggleLeft("隐藏Inspector信息(不隐藏可以在编辑器内修改)", setting.hide_asset_properties);
                setting.use_public_items_getter = EditorGUILayout.ToggleLeft("是否生成获取sheet中所有条目的方法", setting.use_public_items_getter);
                EditorGUILayout.EndVertical();
                EditorGUILayout.BeginVertical();
                setting.compress_color_into_int = EditorGUILayout.ToggleLeft("把颜色数据压缩成整型存储", setting.compress_color_into_int);
                setting.treat_unknown_types_as_enum = EditorGUILayout.ToggleLeft("将未知类型的数据按枚举类型处理", setting.treat_unknown_types_as_enum);
                setting.generate_tostring_method = EditorGUILayout.ToggleLeft("是否生成ToString方法(建议勾选)", setting.generate_tostring_method);
                EditorGUILayout.EndVertical();
                EditorGUILayout.EndHorizontal();
                EditorGUILayout.EndVertical();
                ...
                EditorGUILayout.BeginVertical(GUILayout.Width(100f));
                GUILayout.Space(2f);
                if (GUILayout.Button("插入", GUILayout.Height(20f)))
                {
                    insertAt = i;
                }
                cachedGUIBGColor = GUI.backgroundColor;
                GUI.backgroundColor = Color.red;
                if (GUILayout.Button("删除", GUILayout.Height(20f)))
                {
                    removeAt = i;
                }
                GUILayout.Space(8f);
                ...
                if (GUILayout.Button("转换该Excel", GUILayout.Height(30f)) || (processable && processAll))
                {
